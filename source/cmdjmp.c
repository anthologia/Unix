...
// 					이를 기준으로 한 줄에 출력할 수 있는 열(column)의 개수를 결정
static void
...
void
Sleep(void) { //sleep()은 기존 라이브러리에 존재하므로 Sleep()으로 구현한다.
	int sec;
	sscanf(argv[0], "%d", &sec);
	sleep(sec);
}
...
main(int argc, char *argv[]) {
	...
	help();						// 명령어 전체 리스트 출력
	getcwd(cur_work_dir, SZ_STR_BUF); //현재 작업 디렉토리 이름을 구해와 
									  //cur_wok_dir[]에 저장
